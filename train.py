...
gradient_accumulation_steps = 8  # effective batch = 8 * 32,768 = 262,144 tokens / update
eval_interval = 500
eval_iters = 50
log_interval = 20
max_iters = 20000                # WSD; wallclock-bound (decay auto-triggers near the end)
...
def compute_masked_loss(x_0, t):
    ...
    with ctx:
        logits = model(x_t, t)
        ce = F.cross_entropy(logits.view(-1, args.vocab_size + 1),
                             x_0.view(-1), reduction='none').view(B, T)
...
for it in range(max_iters):
    ...
    if it % eval_interval == 0 or it == max_iters - 1:
        ...
        if losses['val'] < best_val_loss:
            ...
            torch.save({'model': raw.state_dict(), 'args': args, 'step': it}, os.path.join(out_dir, 'ckpt.pt'))
            print("     [checkpoint saved]")
        if decode is not None:
            ids = model.sample(batch_size=1, seq_len=120, temperature=0.8, top_k=15, num_steps=64)
